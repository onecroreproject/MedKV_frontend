...
import { Mic, MicOff, Video, VideoOff, MonitorUp, Square, Circle, Hand, PhoneOff, MessageSquare, Minimize, Maximize } from 'lucide-react';
...
const ClassroomControls = React.memo(({ 
  ...
  return (
    <footer className="pt-3 pb-10 md:py-0 h-auto md:h-20 bg-slate-900 border-t border-slate-800 flex items-center justify-between px-2 md:px-8 z-20 shrink-0">
      ...
      <div className="flex justify-end gap-3 sm:w-1/4">
        <button onClick={onToggleFullscreen} className={`relative p-2.5 md:p-3.5 rounded-xl transition-all border ${isFullscreen ? 'bg-primary text-white border-primary' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200 border-slate-700'}`} title={isFullscreen ? 'Minimize' : 'Maximize'}>
